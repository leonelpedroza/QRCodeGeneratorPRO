...
Developed by LGP with a lot of help from Claude/Anthropic LLM

Tech used is Python and PySide6
...
Desarrollado por LGP con muchisima ayuda de Claude/Anthropic LLM

Tecnología usada es Python y PySide6
...
class QRGeneratorApp(QMainWindow):
    ...
    def _create_settings_tab(self):
        ...
        ec_levels = [
            (self.tr('low_ec'), qrcode.constants.ERROR_CORRECT_L),
            (self.tr('medium_ec'), qrcode.constants.ERROR_CORRECT_M),
            (self.tr('quartile_ec'), qrcode.constants.ERROR_CORRECT_Q),
            (self.tr('high_ec'), qrcode.constants.ERROR_CORRECT_H)
        ]
        
        for i, (text, level) in enumerate(ec_levels):
            radio = QRadioButton(text)
            radio.clicked.connect(self._on_ec_changed)
